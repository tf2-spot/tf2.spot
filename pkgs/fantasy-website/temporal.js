window.addEventListener("load", () => {
  ...
  Array.from(document.getElementsByClassName("temporal")).forEach((el) => {
    let ts = Temporal.Instant.from(el.dataset.ts).toZonedDateTimeISO(
      Temporal.Now.timeZoneId(),
    );

    let tsText = ts.toLocaleString("en-US", {
      dateStyle: "long",
      timeStyle: "short",
    });
